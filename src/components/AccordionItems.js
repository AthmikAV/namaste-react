const AccordianItems = ({ element }) => {
  ...
  const price =
    element?.card?.info?.price || element?.card?.info?.defaultPrice;
  ...
  return (
    <div className="flex justify-between items-start py-4 px-4 border-b last:border-none hover:bg-gray-50 transition-colors">
      ...
      <div className="flex flex-col gap-1 max-w-[70%]">
        ...
        <p className="text-gray-700 font-medium text-sm">₹{price / 100}</p>
        {description && (
      ...
      <div className="relative w-28 h-28">
        ...
        {/* Centered ADD Button */}
        <button className="absolute bottom-1 left-1/2  -translate-x-1/2 bg-blue-600 text-white font-bold px-6 py-1 rounded-lg shadow-md hover:shadow-lg transition cursor-pointer">
          ADD
        </button>
      </div>
    </div>
